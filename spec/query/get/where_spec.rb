...
describe TinyORM::Query::Where do
  ...
  let(:where) { described_class.new(TinyORM::Query::Get::Container.new(Model.new('users'))) }
  ...
  context 'object as argument' do
    let(:subwhere) { TinyORM::Query::Where.new(TinyORM::Query::Get::Container.new(Model.new('books'))) }
